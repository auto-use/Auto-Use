...
class MilestoneService:
    def __init__(self, cli_mode: bool = False, session_id: str = None):
        ...
        # Go up two levels from Auto_Use/macOS_use/controller/milestone/ to reach macOS_use/
        macos_use_dir = os.path.dirname(os.path.dirname(current_dir))

        if cli_mode:
            if session_id:
                self.milestone_dir = os.path.join(macos_use_dir, "scratchpad", "cli_milestone", session_id)
            else:
                self.milestone_dir = os.path.join(macos_use_dir, "scratchpad", "cli_milestone")
        else:
            self.milestone_dir = os.path.join(macos_use_dir, "scratchpad", "milestone")
