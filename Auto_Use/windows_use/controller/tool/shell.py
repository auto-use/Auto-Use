...
class ShellService:
    """Lightweight shell for main agent - quick PowerShell commands via sandboxed Desktop environment"""
    
    def __init__(self):
        """Initialize ShellService with its own Sandbox instance, CWD = Desktop"""
        self.sandbox = Sandbox(workspace_name=".", session_id=None)
        # Override working_dir to Desktop directly (not a subfolder)
        self.sandbox.working_dir = str(Path.home() / "Desktop")
    
    def run(self, command: str, input_text: str = None) -> dict:
        """
        Execute a PowerShell command and return formatted result.
